...
} from "~/lib/projects/opencode.client";
import type {
...
export function SessionTranscript({
  ...
  const loadMoreHistory = useCallback(async () => {
    ...
    try {
      const page = await listOpencodeMessagePageClient(projectId, requestSessionId, {
  ...
  }, [historyCursor, projectId, requestRevalidation]);
  ...
  const replyPermission = useCallback(
    async (requestId: string, reply: "once" | "always" | "reject") => {
      ...
      try {
        await replyToOpencodePermissionRequest(projectId, requestId, reply);
      } catch {
        requestRevalidation();
      }
    },
    [projectId, requestRevalidation],
  );
  ...
  const replyQuestion = useCallback(
    async (requestId: string, answers: OpencodeQuestionAnswer[]) => {
      ...
      try {
        await replyToOpencodeQuestionRequest(projectId, requestId, answers);
      } catch {
        requestRevalidation();
      }
    },
  ...
  const rejectQuestion = useCallback(
    async (requestId: string) => {
      ...
      try {
        await rejectOpencodeQuestionRequest(projectId, requestId);
      } catch {
        requestRevalidation();
      }
    },
  ...
  useProjectEvents(
    ...
    { projectId, sessionId: session.id, types: TRANSCRIPT_EVENT_TYPES },
  );
